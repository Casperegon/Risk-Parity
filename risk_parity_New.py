...
def sample_covariance(returns):
    """
    Standard rolling sample covariance, annualized.

    Parameters
    ----------
    returns : pd.DataFrame or np.ndarray
        Window of daily returns with shape (T, N).

    Returns
    -------
    cov_matrix : np.ndarray
        Annualized covariance matrix with shape (N, N).
    """
    vals = returns.values if hasattr(returns, 'values') else returns
    return np.cov(vals.T) * 252


def ewma_covariance(returns, lambda_param=0.94):
    """
    Exponentially weighted covariance matrix, annualized.

    Gives more weight to recent observations. Lambda controls the decay:
    higher lambda = slower decay = longer memory.
    A common choice is lambda=0.94 (RiskMetrics daily model).
    ...
    -------
    cov_matrix : np.ndarray
        Annualized EWMA covariance matrix with shape (N, N).
    """
    vals = returns.values if hasattr(returns, 'values') else returns
    n = len(vals)

    # Exponential weights: oldest gets lowest weight, newest gets highest
    weights = np.array([(1 - lambda_param) * lambda_param ** (n - 1 - i) for i in range(n)])
    weights /= weights.sum()  # Normalize so weights sum to 1

    demeaned = vals - vals.mean(axis=0)
...
def ledoit_wolf_covariance(returns):
    """
    Ledoit-Wolf shrinkage estimator — industриstandard til Risk Parity.

    Shrinkager stikprøvekkovariansen mod en struktureret target (identity matrix),
    hvilket reducerer estimeringsfejl markant ved korte vinduer eller mange assets.
    Garanterer positiv definithed.

    Ledoit & Wolf (2004): 'A well-conditioned estimator for large-dimensional
    covariance matrices', Journal of Multivariate Analysis.
    """
    clean = returns.dropna() if hasattr(returns, 'dropna') else pd.DataFrame(returns).dropna() #Dropper NaN værdier, hvilket jeg ikke er sikker på er den korrekte måde at gøre det på
    vals = clean.values if hasattr(clean, 'values') else clean

    lw = LedoitWolf().fit(vals)
...
def rolling_rp_backtest(returns, rebalance_freq='M', lookback_window=252, target_vol=None,
                        cov_estimator=None):
    """
    ...
    target_vol : float or None, default None
        If set (e.g. 0.10 for 10%), weights are scaled so the portfolio hits
        this annualized volatility level. Set to None to use unscaled weights
        that sum to 1.
    cov_estimator : callable or None, default None
        Function with signature f(window_returns) -> np.ndarray that returns
        an annualized covariance matrix. Defaults to sample_covariance.
        Use ewma_covariance (or functools.partial to set lambda) for EWMA.
    ...
    for d in candidate_dates:
        # Find nærmeste handelsdag på eller før månedsslutning
        available = returns.index[returns.index <= d]
        if len(available) > 0:
            rebalance_dates.append(available[-1])

    # Fjern eventuelle dubletter (hvis to candidate dates mapper til samme handelsdag)
    ...
    for rebal_date in rebalance_dates:
        ...
        # Estimate covariance
        estimator = cov_estimator if cov_estimator is not None else sample_covariance
        cov_matrix = estimator(window_returns)
        ...
        # Scale weights to target volatility if specified
        if target_vol is not None:
            current_vol = compute_portfolio_volatility(weights, cov_matrix)
            if current_vol > 1e-10:
                weights = weights * (target_vol / current_vol)
...
def detect_weight_regime_shifts(weights_df, lookback=6, threshold=2.0):
    """
    Detect significant changes in Risk Parity weights over time.

    Uses a z-score approach: flags a period if the total weight change
    exceeds `threshold` standard deviations above the rolling mean.
    More robust than a fixed threshold as it adapts to the portfolio's
    typical turnover level.

    Parameters
    ----------
    weights_df : pd.DataFrame
        Risk Parity weights over time from rolling_rp_backtest.
    lookback : int, default 6
        Rolling window for computing mean and std of weight changes.
    threshold : float, default 2.0
        Number of standard deviations above the rolling mean required
        to flag a regime shift.
    ...
    weight_change_df : pd.DataFrame
        Magnitude of absolute weight changes. Index = dates, columns = assets.
    """
    weight_changes = weights_df.diff().abs()
    total_change = weight_changes.sum(axis=1)

    rolling_mean = total_change.rolling(window=lookback).mean()
    rolling_std = total_change.rolling(window=lookback).std()

    z_score = (total_change - rolling_mean) / (rolling_std + 1e-10)
    regime_shift = (z_score > threshold).astype(int)
